...
class FlxGradient
{
			...
			interpolate:Bool = true):BitmapData
	{
		...
		#if flash
		var colorsInt:Array<UInt> = [for (c in colors) c];
		#else
		var colorsInt:Array<Int> = [for (c in colors) c];
		#end
		shape.graphics.beginGradientFill(GradientType.LINEAR, colorsInt,
			#if flash
			[for (a in gradient.alpha) Std.int(a)],
			#else
			gradient.alpha,
			#end
			gradient.ratio, gradient.matrix, SpreadMethod.PAD, interpolationMethod, 0);
